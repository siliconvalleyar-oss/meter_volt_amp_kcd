#!/usr/bin/env python3
"""
Script para agregar pines jerárquicos al schematic principal
"""
import re
import os
import uuid

PROJ_DIR = "/Users/bee/Documents/kicad/amper_volt_meter"

def get_sheet_info():
    """Obtiene información de cada sheet"""
    main_sch = os.path.join(PROJ_DIR, "amper_volt_meter.kicad_sch")
    with open(main_sch, 'r') as f:
        content = f.read()

    sheets = {}
    # Find all sheet definitions
    sheet_pattern = r'\(sheet\s.*?\(uuid\s+"([^"]+)"\).*?\(property\s+"Sheetname"\s+"([^"]+)".*?\(property\s+"Sheetfile"\s+"([^"]+)"'
    for match in re.finditer(sheet_pattern, content, re.DOTALL):
        uuid_val = match.group(1)
        name = match.group(2)
        filename = match.group(3)
        sheets[name] = {'uuid': uuid_val, 'file': filename}

    return sheets

def get_hierarchical_labels(sch_file):
    """Obtiene las etiquetas jerárquicas de un sub-schematic"""
    with open(sch_file, 'r') as f:
        content = f.read()

    labels = []
    # Pattern for hierarchical_label with name on separate line
    pattern = r'\(hierarchical_label\s*\n\s*\(name\s+"([^"]+)"\)'
    for match in re.finditer(pattern, content):
        name = match.group(1)
        # Find the position after this match
        start_pos = match.end()
        # Look for (at x y rotation) within the next 500 chars
        pos_pattern = r'\(at\s+([\d.]+)\s+([\d.]+)\s+(\d+)\)'
        pos_match = re.search(pos_pattern, content[start_pos:start_pos+500])
        if pos_match:
            labels.append({
                'name': name,
                'x': float(pos_match.group(1)),
                'y': float(pos_match.group(2)),
                'rotation': int(pos_match.group(3))
            })
    return labels

def add_pins_to_sheet(main_content, sheet_uuid, labels):
    """Agrega pines jerárquicos a un sheet en el schematic principal"""
    if not labels:
        return main_content

    # Build the hierarchical labels block
    labels_block = ""
    for label in labels:
        label_uuid = str(uuid.uuid4())
        # Adjust position - place pins on the left or top edge of the sheet
        # For simplicity, we'll place them at the label's original position
        x = label['x']
        y = label['y']
        rotation = label['rotation']

        labels_block += f"""		(hierarchical_label
			(name "{label['name']}")
			(shape bidirectional)
			(at {x} {y} {rotation})
			(fields_autoplaced yes)
			(effects
				(font
					(size 1.27 1.27)
				)
			)
			(uuid "{label_uuid}")
		)
"""

    # Find the sheet and add labels before the instances section
    # Pattern to find the specific sheet by UUID
    sheet_pattern = rf'(\(sheet\s.*?\(uuid\s+"{sheet_uuid}"\).*?)(\(instances)'

    def add_labels(match):
        sheet_part = match.group(1)
        instances_part = match.group(2)
        return sheet_part + labels_block + "\t\t" + instances_part

    main_content = re.sub(sheet_pattern, add_labels, main_content, count=1, flags=re.DOTALL)

    return main_content

def main():
    print("=== Agregando Pines Jerárquicos al Schematic Principal ===\n")

    sheets = get_sheet_info()
    main_sch = os.path.join(PROJ_DIR, "amper_volt_meter.kicad_sch")

    with open(main_sch, 'r') as f:
        main_content = f.read()

    total_pins = 0

    for sheet_name, sheet_info in sheets.items():
        sch_file = os.path.join(PROJ_DIR, sheet_info['file'])

        if not os.path.exists(sch_file):
            print(f"  {sheet_name}: Archivo no encontrado")
            continue

        labels = get_hierarchical_labels(sch_file)

        if not labels:
            print(f"  {sheet_name}: Sin etiquetas jerárquicas")
            continue

        print(f"  {sheet_name}: {len(labels)} pines")
        main_content = add_pins_to_sheet(main_content, sheet_info['uuid'], labels)
        total_pins += len(labels)

    with open(main_sch, 'w') as f:
        f.write(main_content)

    print(f"\nTotal: {total_pins} pines agregados")

if __name__ == "__main__":
    main()
